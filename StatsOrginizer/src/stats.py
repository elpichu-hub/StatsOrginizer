...
def run_daily_stats():
    ...
    # xls or xlsx
    try:
        df = pd.read_excel('User Productivity Summary.xls')
        df.to_excel('User Productivity Summary.xlsx', index=False)
    ...
    try:
        work_book = load_workbook('User Productivity Summary.xlsx')
    except Exception as e:
        print(e)
        return e
    ...
    if stats_type == 'Daily':
        ...
        try:
            work_book.save(f'{start_date_formatted}-{end_date_formatted}.xlsx')
        except Exception as e:
            print(e)
            return e
